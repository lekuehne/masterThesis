...
process_plate <- function(file_path) {
  ...
  #colnames(df_clean) <- c("row", "col", "value", "col" "wavelength") # Rename columns
...
# write to CSV
write.csv(combined_data, "all_data_plate1.csv", row.names = FALSE)
...
summary_stat %>% 
  ...
  geom_errorbar(
    ...
    width = 0.5 ) +
  theme_classic() +
  labs(y="OD 600", x="Time", title = "OD 600 of Soil Sample 1 (LB)")


## OD600 sample 6 all dilutions ----
summary_stat %>% 
  filter(sample %in% c("2") &
           wavelength %in% c("OD600")) %>%
  ggplot(aes(x = elapsed_hours, y = mean_value, color = dilution, wavelength)) +
  geom_line() +
  geom_point() +
  geom_errorbar(
    ...
    width = 0.5 ) +
  theme_classic() +
  labs(y="OD 600", x="Time", title = "OD 600 of Soil Sample 2 (LB)")
